...
def add_arguments(parser):
    ...
    parser.add_argument("--gps_file", "-g",
                        help="Path to the GPS optimized trajectory",
                        required=True)
...
def main(args):
    ...
    with rosbag.Bag(args.output, "w") as bag:
        for i, (time, position, orientation, sigma, sigma_orientation) in enumerate(
            ...
            angle, axis = R_to_angle_axis(T_origin_robot[:3, :3])
            local_point = T_origin_robot[:3, -1]
            ...
            quaternion = R_to_quat_xyzw(T_origin_robot[:3, :3])
            output_msg.pose.pose.orientation = Quaternion(
                x=quaternion[0], y=quaternion[1], z=quaternion[2], w=quaternion[3]
            )
            covariance = np.diag(np.concatenate([np.deg2rad(orientation_sigma_origin),
                                                 xyz_sigma_origin]) ** 2)
