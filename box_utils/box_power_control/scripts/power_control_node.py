...
class PowerControlNode(object):
    def __init__(self):
        ...
        rospy.loginfo(f"[PowerControllerNode] Started. By default, both LiDARs are enabled.")
    ...
    def set_hesai_mode(self, request: SetBoolRequest):
        ...
        hesaiResponse = requests.get(
            "http://" + self.hesai_ip + "/pandar.cgi?action=get&object=lidar_data&key=standbymode"
        )
        ...
        if request.data:
            setEnableResponse = requests.get(
                "http://" + self.hesai_ip + "/pandar.cgi?action=set&object=lidar_data&key=standbymode&value=0"
            )
        ...
        if not request.data:
            setDisableResponse = requests.get(
                "http://" + self.hesai_ip + "/pandar.cgi?action=set&object=lidar_data&key=standbymode&value=1"
            )
