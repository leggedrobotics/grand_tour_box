...
missions_release = [m for m in missions if m.name.startswith("release_")]
missions_raw = [m for m in missions if not m.name.startswith("pub_") and not m.name.startswith("release_")]

UUIDS = {}
for mr in missions_raw:
    for mp in missions_pub:
        if mr.name == mp.name[4:]:
            for mrel in missions_release:
                if mr.name == mrel.name[8:]:
                    print(f"Matched: {mr.name} with {mp.name} and {mrel.name}")
                    UUIDS[mr.name] = {"uuid": str(mr.id), "uuid_pub": str(mp.id), "uuid_release": str(mrel.id)}
