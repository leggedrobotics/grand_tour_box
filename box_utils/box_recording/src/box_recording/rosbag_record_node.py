    ...
    def start_recording(self, request):
        ...
        for bag_name, topics in bag_configs.items():
            ...
            if bag_name == "zed2i" and "svo" in topics:
                ...
                bag_path = bag_path = os.path.join(self.bag_base_path, timestamp + "_" + self.node + "_" + bag_name + ".svo2")
            ...
            if bag_name == "hdr":
                ...
                for bag_name in ["hdr_left", "hdr_right", "hdr_front"]:
                    bag_path = os.path.join(self.bag_base_path, f"{timestamp}_{self.node}_{bag_name}", f"{timestamp}_{self.node}_{bag_name}_0.mcap")
                    self.info_string += f"record_{self.node}_hdr_right----{bag_path},"
                continue
