...
if [ "$robot_name" == "jetson" ] || [ "$robot_name" == "Jetson" ]; then
  ...
  rsync --progress ${user}@192.168.2.51:~/git/grand_tour_box/box_utils/box_recording/data/* ${local_path_to_save_data}
  ...
  rsync --progress ${user}@192.168.2.56:~/git/grand_tour_box/box_utils/box_recording/data/* ${local_path_to_save_data}
