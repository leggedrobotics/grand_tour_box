...
class Saver:
    def __init__(self, folder, save_rosbag=True, rosbag_prefix=""):
        ...
        if save_rosbag:
            bag_path = self.folder / f"{rosbag_prefix}_wavemap.bag"
...
def get_bag(directory, pattern):
    ...
    if len(files) != 1:
        print(f"Error: Found {len(files)} matching bag files for pattern '{pattern}' in directory '{directory}'")
        return None, False
    return files[0], True
...
if __name__ == "__main__":
    ...
    # Retrieve tf bag
    tf_bag_path, success = get_bag(args.mission_data, "*_tf_static_dlio.bag")
    prefix = Path(tf_bag_path).stem.split("_")[0]

    if not success:
        raise FileNotFoundError("TF bag not found.")
    ...
    for camera in config["cameras"]:
        bag_file, success = get_bag(args.mission_data, camera["bag_pattern"])
        if not success:
            print(f"Failed to find bag for {camera['name']}")
            continue
