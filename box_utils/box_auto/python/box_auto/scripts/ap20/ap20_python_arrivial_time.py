...
def read_bag_file(bag_path):
    ...
    data_position = []
    with rosbag.Bag(
        bag_path.replace("jetson_ap20_aux", "jetson_ap20_ros_arrivial_time"), "w", compression="lz4"
    ) as bag_out:
                ...
                if len(data_position) > 0:
                    ...
                    if ref > 0 and ref != -1:
                        ...
                        if not (delta > 0.005 + 1e-5):
                            ...
                            print(
                                "{:<20} {:<15} {:<20} {:<20}".format(
                                    "IMU t-1  --- AP20_TIME:", ti(p1), ", ARRIVAL_TIME:", th(p1)
                                )
                            )
                            print(
                                "{:<20} {:<15} {:<20} {:<20}".format(
                                    "IMU t    --- AP20_TIME:", ti(p2), ", ARRIVAL_TIME:", th(p2)
                                )
                            )
                            print(
                                "{:<20} {:<15} {:<20} {:<20} {:<20} {:<20} ".format(
                                    "Position --- AP20_TIME:",
                                    t,
                                    ", ARRIVAL_TIME:",
                                    th(position),
                            ...
                            counter_ts += 1
                            bag_out.write(
                                "/gt_box/ap20/prism_position_ros_arrival_timestamp", new_msg, new_msg.header.stamp
                            )
