...
def run_eval(test_name, reference_file, estimated_file, params, output_path, filter_config, mode):
    ...
    #     indices_to_remove = np.where(time_diffs > filter_config.get("ap20_peak_rejection_threshold", 0.15))[0]
    ...
    gmsf = gigachad_colors[0]
    orange = gigachad_colors[1]
    tps = gigachad_colors[3]  # "green" #
    fill_alpha = 0.3
    ...
    ax2.plot(
        traj_estimated_original_file.timestamps, std_dev_x, color=orange, label="Std. Dev. X", zorder=4, linestyle="--"
    )
    ax2.set_ylabel("Std. Dev. [m]", color="black", fontsize=20, fontweight="bold")

    yticks_1 = [-20, -10, 0, 10, 20]
    yticks_2 = [0, 0.02, 0.04, 0.06]
    y1_min = yticks_1[0] - 0.15 * (yticks_1[-1] - yticks_1[0])
    y1_max = yticks_1[-1] + 0.15 * (yticks_1[-1] - yticks_1[0])
    y2_min = yticks_2[0] - 0.15 * (yticks_2[-1] - yticks_2[0])
    y2_max = yticks_2[-1] + 0.15 * (yticks_2[-1] - yticks_2[0])
    # Set the y-ticks and y-limits for both axes
    ...
    leg = axs[0].legend(
        combined_handles, combined_labels, loc="upper left", fontsize=16, framealpha=1, edgecolor="black"
    )
    ...
    for handle in leg.legendHandles:
        if isinstance(handle, plt.Line2D):  # If it's a line
            ...
            handle.set_alpha(1.0)  # Adjust transparency

    # plt.grid(b=True, which='major', color='k', linestyle='-')
    axs[0].grid(which="minor", color="r", linestyle="-", alpha=0.2, zorder=0)
    ...
    ax1_y2.plot(traj_estimated_original_file.timestamps, std_dev_y, linestyle="--", color=orange, label="Std. Dev. Y")
    ax1_y2.set_ylabel("Std. Dev. [m]", color="black", fontsize=20, fontweight="bold")

    yticks_1 = [-30, -15, 0, 15, 30]
    yticks_2 = [0, 0.02, 0.04, 0.06]
    y1_min = yticks_1[0] - 0.15 * (yticks_1[-1] - yticks_1[0])
    y1_max = yticks_1[-1] + 0.15 * (yticks_1[-1] - yticks_1[0])
    y2_min = yticks_2[0] - 0.15 * (yticks_2[-1] - yticks_2[0])
    ...
    ax1_y2.set_yticks(yticks_2)
    ax1_y2.set_ylim(y2_min, y2_max)
    ...
    ax2_y2.plot(traj_estimated_original_file.timestamps, std_dev_z, color=orange, label="Std. Dev. Z", linestyle="--")
    ...
    y2_max = yticks_2[-1] + 0.15 * (yticks_2[-1] - yticks_2[0])
    # Set the y-ticks and y-limits for both axes
    axs[2].set_yticks(yticks_1)
    axs[2].set_ylim(y1_min, y1_max)
    ax2_y2.set_yticks(yticks_2)
    ax2_y2.set_ylim(y2_min, y2_max)
    ...
    axs[0].set_facecolor((245 / 255, 245 / 255, 245 / 255))
    axs[1].set_facecolor((245 / 255, 245 / 255, 245 / 255))
    axs[2].set_facecolor((245 / 255, 245 / 255, 245 / 255))

    plt.tight_layout()
    # plt.show()
    plt.savefig(output_path + "/gmsf_scatter_plot_XYZ_" + test_name + ".pdf", dpi=600)
...
if __name__ == "__main__":
    # Construct paths
    CONFIG_PATH = (
        "/home/tutuna/box_ws/src/grand_tour_box/box_utils/box_auto/cfg/evo_evaluations_gt_truth_comparison.yaml"
    )
    INPUT_FOLDER_PATH = "/home/tutuna/Videos/2024-11-14-13-45-37_heap_testsite/2024-11-14-13-45-37_evo_evaluations"
    OUTPUT_DIR_NAME = "/home/tutuna/Videos/2024-11-14-13-45-37_heap_testsite/for_paper/"
    DISABLE_VIZ = False
    PREFIX = "tt_"
