...
def check_clock_offsets(bags):
    ...
    results_filename = os.path.join(MISSION_DATA, "time_check_bag.txt")

    with contextlib.redirect_stdout(TeeFile(results_filename)):
        for bag in bags:
            for topic, msg, _t in rosbag.Bag(bag).read_messages():
...
if __name__ == "__main__":
    check_clock_offsets(
        [str(s) for s in pathlib.Path(MISSION_DATA).rglob("*_nuc_utils.bag")]
