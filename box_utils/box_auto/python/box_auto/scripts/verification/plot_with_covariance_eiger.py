...
def run_eval(test_name, reference_file, estimated_file, params, output_path, filter_config, mode):
    ...
    #     # Remove the measurements around the lost RTS Measurements measurements
    ...
    if size_of_elements_after_association < 0.5 * size_of_elements_before_association:
        ...
        print(
            "\033[91mLess than 50% match. Either time synchronization parameters for association is wrong or a lot of missing RTS Measurements segments.\033[0m"
        )
    ...
    axs[0].fill_between(
        traj_estimated_original_file.timestamps,
        traj_estimated_original_file.positions_xyz[:, 0] - (std_dev_x * 100),
        traj_estimated_original_file.positions_xyz[:, 0] + (std_dev_x * 100),
        color="green",
        alpha=0.3,
        label="100x Std. Dev. X",
        zorder=1,
    )
    ...
    leg = axs[0].legend(
        combined_handles, combined_labels, loc="upper center", fontsize=16, framealpha=1, edgecolor="black"
    )
    ...
    axs[1].fill_between(
        traj_estimated_original_file.timestamps,
        traj_estimated_original_file.positions_xyz[:, 1] - (std_dev_y * 100),
        traj_estimated_original_file.positions_xyz[:, 1] + (std_dev_y * 100),
        color="green",
        alpha=0.3,
        label="100x Std. Dev. Y",
        zorder=1,
    )
    ...
    axs[2].fill_between(
        traj_estimated_original_file.timestamps,
        traj_estimated_original_file.positions_xyz[:, 2] - (std_dev_z * 100),
        traj_estimated_original_file.positions_xyz[:, 2] + (std_dev_z * 100),
        color="green",
        alpha=0.3,
        label="100x Std. Dev. Z",
    )
    ...
    # axs[0].set_ylim(-18, 21)
    # axs[1].set_ylim(-21, 38)
    # axs[2].set_ylim(-2, 3)
    ...
    plt.tight_layout()
    plt.savefig(output_path + "/scatter_plot_XYZ_" + test_name + ".pdf", dpi=600)
    plt.close()
