...
for delay in x:
    filename_ape = generate_filename(base_ape, "ape_results_dlio_hesai_new_formulation", delay)
    filename_rpe = generate_filename(base_rpe, "RPE_results_dlio_hesai_new_formulation", delay)
...
axs[0].set_xlim(min(x_plotting) - 1, max(x_plotting) + 1)
...
axs[1].set_xlim(min(x_plotting) - 1, max(x_plotting) + 1)
# Grid for better readability
...
fig.savefig(os.path.join(output_dir, "time_offset_abliation.pdf"), format="pdf", bbox_inches="tight", dpi=600)
