...
def run_ate(test_name, reference_file, estimated_file, params, output_path, disable_viz, relation):
    ...
    results = []
    result = main_rpe.rpe(traj_reference, traj_estimated, est_name=test_name, ref_name="IE", 
                            pose_relation=relation, delta=delta, delta_unit=delta_unit, 
    ...
    fig.suptitle("Absolute Pose Error (RPE) Trajectory", fontsize=25)
    ...
    if relation == PoseRelation.translation_part:
        cbar.set_label('RPE [m]', labelpad=20)
    elif relation == PoseRelation.rotation_angle_deg:
        cbar.set_label('RPE [deg]', labelpad=20)
    ...
    file_interface.save_res_file(output_path + "/RPE_results_" + test_name+ ".zip", results[0],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run RPE evaluation for multiple tests.")
