...
def generate_filename(base, prefix, delay):
    ...
    return f"{base}{prefix}_{sign}{ms_str}mm_tp_all.zip"
...
x = [-55, -50, -45, -40, -35, -30, -25, -20, -15, -10, -5, -2, 0, 2, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55]
...
for delay in x:
    filename_ape = generate_filename(base_ape, "ape_results_dlio_trans", delay)
    filename_rpe = generate_filename(base_rpe, "RPE_results_dlio_trans", delay)
...
for delay in x:
    ...
    if rpe_data:
        ...
        yerr_rpe.append(rpe_data.stats["std"] / 2.0)
...
axs[1].set_xlabel("Translation Offset Value [mm]", fontsize=20, fontweight="bold")
...
axs[1].set_xlim(min(x) - 1, max(x) + 1)
# Grid for better readability
...
fig.savefig(os.path.join(output_dir, "translation_offset_abliation.pdf"), format="pdf", bbox_inches="tight", dpi=600)
