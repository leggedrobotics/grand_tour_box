...
for p in [str(s) for s in p.glob("*tf_static_metadata_*.bag") if "new" not in str(s)]:
    ...
    tf_listener_urdf = BagTfTransformer("/home/jonfrey/2025-04-02-17-34-55.bag")
        ...
        with rosbag.Bag(p.replace("tf_static_metadata_", "tf_static_metadata_new_"), "w", compression="lz4") as out_bag:
            for topic, msg, _t in bag.read_messages():
                if topic == "/tf_static":
                    ...
                    tf = tf_listener_urdf.lookupTransform("box_base", "box_base_model", time=None, latest=True)
                    out = copy.deepcopy(msg.transforms[0])
                    out.header.frame_id = "box_base"
                    ...
                    tf = tf_listener_urdf.lookupTransform("base", "cpt7_antenna_back_model", time=None, latest=True)
                    out = copy.deepcopy(msg.transforms[0])
                    ...
                    # Rename frame_id + flip tf
                    tf = tf_listener_urdf.lookupTransform("box_base", "prism_model", time=None, latest=True)
                    out = copy.deepcopy(msg.transforms[0])
                    out.header.frame_id = "box_base"
                    ...
                    tf = tf_listener_urdf.lookupTransform("box_base", "prism", time=None, latest=True)
                    out = copy.deepcopy(msg.transforms[0])
                    out.header.frame_id = "box_base"
