...
def _load_metadata_from_mcap_file_and_topic(
    mcap_path: Path,
    topic_desc: Topic,
) -> Dict[str, Any]:
...
def _load_camera_info_metadata_from_mcap_file_and_topic(
    mcap_path: Path,
...
def _load_tf_metadata_from_mcap_file_and_topic(
    mcap_path: Path,
...
def _get_frame_id_from_topic(mcaps_path: Path, topic_desc: Topic) -> Dict[str, str]:
    mcap_path = mcaps_path / topic_desc.file
    ret = _load_metadata_from_mcap_file_and_topic(mcap_path, topic_desc)
    return ret
...
def _get_frame_ids(mcaps_path: Path, topic_reg: TopicRegistry) -> Dict[str, Any]:
    ret = {}
    for _, topic_desc in topic_reg.values():
        ret[topic_desc.alias] = _get_frame_id_from_topic(mcaps_path, topic_desc)
    return ret
