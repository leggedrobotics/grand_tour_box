...
for j, p in enumerate(missions):
    print(j)
    mission_data = base / p
    output_file = str(mission_data / "output_okvis_export.log")
    cmd = f"export MISSION_DATA={mission_data}; docker compose -f /home/jonfrey/git/grand_tour_box/box_utils/box_auto/compose/okvis.yaml up --no-deps okvis_export_all  > {output_file} 2>&1"
    print(cmd)
    os.system(cmd)
