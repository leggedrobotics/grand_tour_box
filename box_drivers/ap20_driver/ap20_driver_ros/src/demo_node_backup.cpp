...
private:
    ...
    // Mutexes
    std::mutex imu_message_queuemutex_;
    std::mutex position_message_queuemutex_;
    ...
    void initializePublishersAndSubscribers() {
        ap20_imu_pub_ = nh_.advertise<sensor_msgs::Imu>("imu", 200);
        ap20_position_pub_ = nh_.advertise<geometry_msgs::PointStamped>("prism_position", 100);
...
void AP20Node::imuCallback(const sensor_msgs::Imu::ConstPtr& msg) {
    std::lock_guard<std::mutex> lock(imu_message_queuemutex_);
...
void AP20Node::positionCallback(const geometry_msgs::PointStamped::ConstPtr& msg) {
    std::lock_guard<std::mutex> lock(position_message_queuemutex_);
...
void AP20Node::publishPositionMessages() {
    ...
    if (!position_message_queue.empty()) {
        auto position = position_message_queue.back();
        ...
        if (j == lookup_ts_.size()) {
            position_message_queue.push_front(position);
            return;
        }
...
void AP20Node::publishImuMessages() {
    ...
    if (!timestamp_queue_.empty() && !imu_message_queue.empty()) {

        ROS_INFO_STREAM("Timestamp queue size: " << timestamp_queue_.size());
        ROS_INFO_STREAM("IMU message queue size: " << imu_message_queue.size());

        auto time_msg = timestamp_queue_.back();
        timestamp_queue_.pop_back();
        auto imu = imu_message_queue.back();
...
void AP20Node::run() {
    ...
    while (ros::ok()) {
        while (true) {
            ...
            // timestamp_reader_->readTimestamps();
            ROS_INFO_STREAM("Timestamp queue size: " << old_timestamps_.size());
