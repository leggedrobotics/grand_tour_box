...
class AP20Node:
    def __init__(self):
        ...
        self.timestamps_fn = f"/sys/kernel/time_stamper/ts_buffer"
    ...
    def change_imu_mode(self, enable_streaming):
        ...
        try:
            ...
            if response.success:
                rospy.loginfo(f"IMU mode changed successfully. Streaming is now {'enabled' if enable_streaming else 'disabled'}.")
                return True
            else:
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            while True:
                ...
                old_timestamps = self.timestamp_reader.read_timestamps()
            ...
            while True:
                if self.imu_message_counter % 1000 == 0:
                    print(f"IMU messages: {self.imu_message_counter} - Timestamps: {self.timestamp_counter} - Position messages: {self.position_message_counter}")
        ...
        if self.timestamp_queue and self.imu_message_queue:
            ...
            if len(self.line_delay) == 20:
                ...
                if abs(est - new_line_delay) > 0.005:
                    self.shutdown = True
                    rospy.logerr(f"Line delay is too high. Time skipp detected.")
