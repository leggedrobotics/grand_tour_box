...
DURATION=${1:-10}  # Default to 1800 seconds (30 minutes) if no argument is provided
...
sudo iftop -t -i enp45s0 > "$OUTPUT_DIR/network_usage_enp45s0.txt" &
IFTOP_PID1=$!
echo "Started network monitoring on enp45s0 (PID: $IFTOP_PID1)"

# Network profiling for interface enp46s0 (run in background)
sudo iftop -t -i enp46s0 > "$OUTPUT_DIR/network_usage_enp46s0.txt" &
IFTOP_PID2=$!
...
iostat -dx nvme0n1 1 "$DURATION" > "$OUTPUT_DIR/disk_write_speed_nvme0n1.log" &
