...
void RerunCameraPrismViz::logStripAndTimeSeries(
    ...
    rec_.log(strip_entity, rerun::LineStrips3D(pts).with_colors(color));
    ...
    for (size_t i = 0; i < timestamps_s.size(); ++i) {
        rec_.set_time_timestamp_secs_since_epoch("time", timestamps_s[i]);
...
void RerunCameraPrismViz::vizStaticFrames(const Eigen::Affine3d& T_leica_board) {
    rec_.log_static(base_name_, rerun::ViewCoordinates::RIGHT_HAND_Y_DOWN);
